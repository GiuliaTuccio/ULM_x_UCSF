function [ tracks adjacency_tracks A ] = simpletrackerV2(points, frameRate, varargin)
...
% Jean-Yves Tinevez < jeanyves.tinevez@gmail.com> November 2011 - 2012
    ...
    p.addParamValue('MaxDistanceZX', defaultMaxDistanceZX, @isnumeric);
    ...
    MaxDistanceZX           = p.Results.MaxDistanceZX;
...
%             fprintf('%03d/%03d', i, n_slices-1);
        ...
        switch lower(method)
            ...
            case 'hungarian'
                [target_indices , ~, unmatched_targets{i+1} ] = ...
                    hungarianlinker(source, target, max_linking_distance,MaxDistanceZX);
        ...
        time = [0 1/frameRate];
        vu=[];
        for i=1:size(target_indices,2)
            pairs = cat(1, source(i,:), target(target_indices(i),:));
            vu = cat(2,vu,[diff(pairs(:,1))./diff(time);...
                diff(pairs(:,2))./diff(time)]); 
            dy = pairs(2,1)-pairs(1,1);
            dx = pairs(2,2)-pairs(1,2);
            theta_rad = atan2(dy, dx);  % Angle in radians
            theta_deg(i) = rad2deg(theta_rad);  % Angle in degrees
            %vxu = diff(pairs(:,2))./diff(time);
            %v(i)=smooth(vecnorm([vzu vxu],2,2),10);
        end
        v=vecnorm(vu);
        
        %alpharad = acos(dot(vu(:,1), vu(:,1)) / sqrt( dot(vu(:,1), veca) * dot(vecb, vecb)));
