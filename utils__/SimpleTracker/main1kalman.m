...
addpath(genpath('challenge_data'))
addpath(genpath('HDBSCAN-master'))
% Read video file
vid = VideoReader('D:\phD\Optical_flow\challenge_data\simu1\net_002.avi');
video = read(vid);

% Select first frame
frame = double(squeeze(video(:,:,1,:)));

% Load metadata
load('D:\phD\Optical_flow\challenge_data\simu1\metadata.mat');
point = readmatrix('D:\phD\Optical_flow\challenge_data\simu1\gt_scat_inside.txt');

tracks_id = [681]';%unique(point(:,2));
points=[];
for t=1:size(tracks_id,1)
    p_idx = point(:,2)==tracks_id(t);
    points = cat(1,points,point(p_idx,:));
end
nFrames = length(unique(points(:,1)));
detections = cell(nFrames,1);
for f=1:nFrames
    idx = points(:,1)==f;
    detections{f}=[ (points(idx,3)-PxSet.mapX(1))./PxSet.dx (points(idx,5)-PxSet.mapZ(1))./PxSet.dz];
end

%% define main variables
dt = 1;  %our sampling rate
S_frame = 1; %starting frame
u = .0005; % define acceleration magnitude
Q= [detections{S_frame,1}(:,1); detections{S_frame,1}(:,2); 0; 0]; %initized state--it has four components: [positionX; positionY; velocityX; velocityY] of the hexbug
...
for t = S_frame:size(detections,1)
    
    % load the image
    %img_tmp = double(imread(f_list(t).name));
    img = frame(:,:,t);
    % load the given tracking
    Q_loc_meas(:,t) = [ detections{t,1}(:,2); detections{t,1}(:,1)];
